...
func (s *budgetService) GenerateAndSendBudget(ctx context.Context, workOrderID uuid.UUID) error {
	...
	for _, svc := range services {
		serviceItems = append(serviceItems, email.BudgetServiceItem{
			...
			ApproveLink: fmt.Sprintf("%s/work-orders/services/%s/approve", s.baseURL, svc.ID),
			RejectLink:  fmt.Sprintf("%s/work-orders/services/%s/reject", s.baseURL, svc.ID),
		})
	}
	...
	data := email.BudgetEmailData{
		...
		ApproveAllLink: fmt.Sprintf("%s/work-orders/%s/approve-all", s.baseURL, workOrderID),
		RejectAllLink:  fmt.Sprintf("%s/work-orders/%s/reject-all", s.baseURL, workOrderID),
	}
